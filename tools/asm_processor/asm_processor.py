...
class Section:
    ...
    def init_symbols(self, sections):
        ...
        entries = [
            Symbol(self.data[i : i + self.sh_entsize], self.strtab)
    ...
    def init_relocs(self):
        assert self.is_rel()
        entries = [
            Relocation(self.data[i : i + self.sh_entsize], self.sh_type)
            for i in range(0, self.sh_size, self.sh_entsize)
        ]
    ...
    def relocate_mdebug(self, original_offset):
        ...
        (
            ...
            hdrr_iextMax,
            hdrr_cbExtOffset,
        ) = struct.unpack(">HHIIIIIIIIIIIIIIIIIIIIIII", self.data[:0x60])
